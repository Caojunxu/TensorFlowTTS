...
from tensorflow_tts.processor.Chinese import Chinese_SYMBOLS
...
def parse_and_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ljspeech",
        choices=["ljspeech", "kss", "libritts", "baker","Chinese"],
...
def gen_audio_features(item, config):
    ...
    # check sample rate
    # print(rate,config['sampling_rate'])
...
def preprocess():
    ...
    processor._save_mapper(
        ...
        if config["dataset"] == "baker" or config["dataset"] == "Chinese"
        else {},
    )
    ...
    id_to_remove = []
    f = open(os.path.join(config["outdir"], "phoneme.txt"), 'w', encoding='utf-8')
    for result, mel, energy, f0, features in train_map:
        ...
        utt_id = features["utt_id"]
        chn_char = features["chn_char"]
        phoneme = features["raw_text"]
        f.write(chn_char+'\n'+phoneme+'\n\n')
